  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-baseline p-6">
      ...
      <section className="w-full max-w-6xl mt-6">
        <h1 className="font-bold font-doto text-3xl">
        ...
        <Table>
          ...
          <TableBody>
            {currentRows.map((item, index) => (
              <TableRow key={item._id} className={index % 2 && "bg-primary/8" || ""}>
                <TableCell>{formatDate(item.date ? new Date(item.date) : undefined)}</TableCell>
        ...
        <div className="w-full flex items-center justify-center gap-4 mt-4 max-w-lg mx-auto">
            ...
            <p className="text-sm text-white/85 flex-grow text-center">
              Page <span className="text-primary font-bold text-[15px]">{currentPage}</span> of {totalPages}
            </p>
